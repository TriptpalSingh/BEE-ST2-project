...
const conn = mysql.createConnection({host:'localhost', user:'root', password:'', database:'data2'});
...
app.post("/signupform-submit",function(req,res){
    ...
    conn.connect(function(error){
        ...
        var sql = "SELECT * FROM user_info WHERE Username = "+mysql.escape(Username);
        conn.query(sql,function(error,result){
            if(error) throw error;
            if(result.length==0){
                var sql = "INSERT INTO user_info(`Username`,`Password`,`FatherName`,`PhoneNumber`,`Gender`,`Address`) VALUES('"+Username+"','"+Password+"','"+Fname+"','"+PhoneNumber+"','"+Gender+"','"+Address+"')";
                conn.query(sql,function(error,result){
...
app.post("/Profile", function(req, res){
    ...
    conn.connect(function(error){
        ...
        var sql = "SELECT * FROM user_info WHERE Username = "+mysql.escape(Username)+" AND Password = "+mysql.escape(Password);
        conn.query(sql,function(error,result){
            if(error) throw error;
            if(result.length==0){
                req.session.message = {
...
app.post("/ProfileUpdated", function(req, res){
    ...
    conn.connect(function(error){
        ...
        var sql = "UPDATE user_info SET Password = "+mysql.escape(NewPassword)+" WHERE Password = "+mysql.escape(OldPassword);
        conn.query(sql,function(error,result){
            if(error) throw error;
            console.log("Password Updated.");
            var sql = "SELECT * FROM user_info WHERE Password = "+mysql.escape(NewPassword);
            conn.query(sql,function(error,result){
                if(error) throw error;
                req.session.message = {
                    type : 'success',
